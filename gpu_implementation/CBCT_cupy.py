# CBCT (Cone Beam Computed Tomography) GPU Reconstruction Script
# --------------------------------------------------------------
# This file implements a modular pipeline for CBCT image reconstruction using GPU acceleration.
# It consolidates: 
#   - configuration management,
#   - projection data loading,
#   - preprocessing,
#   - GPU memory management,
#   - filtering (ramp filter per detector row),
#   - 3D cone beam backprojection,
#   - orchestration and result saving.
# 
# All classes and functions feature clear, extensible comments and standardized naming conventions 
# for maintainability and ease of future modification.
...
# Configure logging for standardized output across the module.
...
class CBCTConfig:
    """
    CBCT reconstruction configuration parameters.
    All physical and processing parameters are defined here for easy tuning.
    This object can be extended to add more configuration fields as needed.
    """
    num_projections: int = 1600
    scan_angle: float = 360.0
    start_angle: float = 270.0
    detector_size: Tuple[int, int] = (2860, 2860)  # (height, width)
    ...
    filter_row_chunk: int = 16
    filter_angle_chunk: int = 64
    max_gpu_memory_fraction: float = 0.8


class GPUMemoryManager:
    """
    Utilities for GPU memory querying and cleanup.
    Ensures that GPU memory use is monitored and cleaned up between operations.
    """
    @staticmethod
    def get_memory_info() -> Dict[str, float]:
        """
        Query and return GPU memory statistics.
        Returns a dictionary with used, total, free, and pool memory in GB.
        """
        try:
            ...
            # On failure, return zeros to force CPU fallback if needed.
            return {'used': 0.0, 'pool_total': 0.0, 'device_free': 0.0, 'device_total': 0.0}

    @staticmethod
    def cleanup():
        """
        Free all memory blocks from the default and pinned memory pools.
        """
        try:
    ...
    def estimate_sinogram_memory(shape: Tuple[int, ...]) -> Dict[str, float]:
        """
        Estimate memory requirements for sinogram filtering and backprojection.
        Accepts 2D (projLen, numAngles) or 3D (H, W, N) shapes.
        Returns memory estimates in GB.
        """
        if len(shape) == 2:
            proj_len, num_angles = shape
            num_rows = 1
        elif len(shape) == 3:
            num_rows, proj_len, num_angles = shape
        else:
            raise ValueError("Unsupported shape for memory estimation")

        sino_mem = proj_len * num_angles * num_rows * 4 / 1e9      # float32
        fft_temp = proj_len * num_angles * num_rows * 8 / 1e9      # complex64
        backproj_mem = proj_len * proj_len * 4 / 1e9               # estimate
...
class CBCTPreprocessor:
    """
    Projection preprocessor for CBCT.
    Handles dark current subtraction, log correction, bad pixel masking, noise reduction, and edge/truncation correction.
    """
    ...
    def preprocess_projection_gpu(self, proj_gpu: cp.ndarray) -> cp.ndarray:
        """
        Preprocess a single projection image on GPU.
        Steps: dark subtraction, log transform, bad pixel correction, noise reduction, truncation correction.
        """
        try:
    ...
    def _bad_pixel_correction_gpu(self, proj_gpu: cp.ndarray) -> cp.ndarray:
        """
        Apply median filtering to pixels above threshold (bad pixels).
        """
        try:
    ...
    def _noise_reduction_gpu(self, proj_gpu: cp.ndarray) -> cp.ndarray:
        """
        Smooth projection using a Gaussian filter.
        """
        try:
    ...
    def _truncation_correction_gpu(self, proj_gpu: cp.ndarray) -> cp.ndarray:
        """
        Correct edge truncation by setting border regions to average edge value.
        """
        H, W = proj_gpu.shape
...
class CBCTDataLoader:
    """
    Loads CBCT projection stacks from disk and applies preprocessing.
    Projects are loaded in batches for memory safety.
    """
    def __init__(self, config: CBCTConfig):
        self.config = config
    ...
    def load_projections(self, proj_folder: str, start_num: int = 0) -> cp.ndarray:
        """
        Load all projections from the given folder, preprocess, and return as a GPU array.
        Returns an array of shape (H, W, N).
        """
        ...
        for batch_idx in tqdm(range(num_batches), desc="Loading projection batches"):
            ...
            try:
                batch_data = self._load_projection_batch(proj_folder, start_num, batch_start, batch_end, (H, W))
    ...
    def _load_projection_batch(self, proj_folder: str, start_num: int, batch_start: int,
                              batch_end: int, proj_shape: Tuple[int, int]) -> cp.ndarray:
        """
        Load a batch of projections and preprocess each one.
        Handles missing or corrupt files gracefully.
        Returns a GPU array of shape (H, W, batch_size).
        """
        H, W = proj_shape
        batch_projections = []
...
class CBCTRampFilter:
    """
    Applies the 1D ramp filter in the frequency domain to each detector row of the 3D projection stack.
    Filtering is performed in row chunks to avoid memory overuse.
    """
    def __init__(self, config: CBCTConfig):
        self.config = config
    ...
    def filter_projections(self, projections: cp.ndarray) -> cp.ndarray:
        """
        Apply the ramp filter to the input projections (shape: H, W, N).
        Returns a filtered array of the same shape.
        """
        ...
        filt_gpu = self._create_ramp_filter(W)
    ...
    def _create_ramp_filter(self, proj_len: int) -> cp.ndarray:
        """
        Create the frequency-domain ramp filter for filtering projections.
        """
        a = 0.5
        step = 2 * np.pi / proj_len
...
class CBCTBackprojector3D:
    """
    Implements the 3D cone beam backprojection using a custom CUDA kernel.
    This class is responsible for mapping filtered projections to a reconstructed 3D volume.
    """
    def __init__(self, config: CBCTConfig):
        self.config = config
    ...
    def _compile_backprojection_kernel(self):
        """
        Compile the custom raw CUDA kernel for 3D cone beam backprojection.
        The kernel uses bilinear interpolation on the detector.
        """
        kernel_source = r"""
        extern "C" __global__
        ...
        """
        self._backproj_3d_kernel = cp.RawKernel(kernel_source, "backproj_3d_kernel")

    def _backproject(self, projections: cp.ndarray, angles: np.ndarray) -> np.ndarray:
        """
        Perform 3D cone beam backprojection from filtered projections.
        Returns a reconstructed volume as a numpy array.
        """
...
class CBCTReconstructionPipeline:
    """
    Main orchestration class for CBCT reconstruction.
    Loads, preprocesses, filters, backprojects, and saves results.
    """
    def __init__(self, config: CBCTConfig):
        self.config = config
        self.data_loader = CBCTDataLoader(config)
        self.filter = CBCTRampFilter(config)
        self.backprojector = CBCTBackprojector3D(config)

    def reconstruct(self, projection_folder: str, output_folder: str) -> Dict[str, Any]:
        """
        Main pipeline for CBCT reconstruction.
        ...
        try:
            ...
            reconstruction = self.backprojector._backproject(filtered, angles)
    ...
    def _initialize_gpu(self):
        """
        Run a dummy operation to initialize GPU and memory pools.
        """
    ...
    def _apply_volume_denoising(self, volume: np.ndarray) -> np.ndarray:
        """
        Apply Gaussian denoising to the reconstructed volume using GPU.
        """
        try:
            from cupyx.scipy.ndimage import gaussian_filter
    ...
    def _save_results(self, reconstruction: np.ndarray, output_folder: str, reconstruction_time: float) -> Dict[str, Any]:
        """
        Save the reconstructed volume and metadata to disk.
        Provides a raw file, metadata file, and preview PNG of the central slice.
        """
...
def main():
    """
    Entry point for CBCT reconstruction.
    Configures and runs the pipeline, logging results and timing.
    """
    config = CBCTConfig(
        num_projections=100,
        scan_angle=360.0,
    ...
    try:
        reconstructor = CBCTReconstructionPipeline(config)
